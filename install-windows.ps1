# Mole Installation Script for Windows
# Installs Mole system maintenance tool
...
[CmdletBinding()]
param(
    [string]$InstallPath = "$env:ProgramFiles\Mole",
    [switch]$Uninstall
)
...
if ($Uninstall) {
    Write-ColorMessage "`nUninstalling Mole..." -Color Magenta
...
Write-ColorMessage "`nMole Installation for Windows" -Color Magenta
...
# Get current directory (should be Mole repo root)
...
if ($createShortcut -eq 'y' -or $createShortcut -eq 'Y') {
    ...
    $Shortcut = $WshShell.CreateShortcut("$env:USERPROFILE\Desktop\Mole.lnk")
